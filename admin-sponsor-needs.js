(()=> {
  ...
  function badge(s){
    const labels={pending_payment:'Pending payment',confirmed:'Confirmed',paid:'Paid / fulfilled',declined:'Declined'};
    return labels[s]||s;
  }
  async function render(){
    ...
    const confirmed=rows.filter(x=>['confirmed','paid'].includes(x.status)).reduce((a,x)=>a+Number(x.amount||0),0);
    const pending=rows.filter(x=>x.status==='pending_payment').reduce((a,x)=>a+Number(x.amount||0),0);
    el.innerHTML=`<div class="admin-section-title"><h2>Specific-need sponsorships</h2><p>Website selections flow here automatically. Only confirmed/paid amounts count toward public fulfillment totals.</p></div>
      <div class="admin-finance-grid">
        <div class="admin-finance-card"><strong>${money(confirmed)}</strong><span>Confirmed / paid</span></div>
      ...
      ${rows.length?rows.map(x=>`<tr><td><b>${esc(x.need_title)}</b><br><small>${new Date(x.created_at).toLocaleString()}</small></td><td>${esc(x.organization||x.donor_name)}<br><small>${esc(x.email)}</small></td><td><b>${money(x.amount)}</b></td><td>${esc(badge(x.status))}</td><td>${esc(x.recognition||'—')}</td><td><select data-sponsor-id="${x.id}" aria-label="Update sponsorship status"><option value="pending_payment" ${x.status==='pending_payment'?'selected':''}>Pending payment</option><option value="confirmed" ${x.status==='confirmed'?'selected':''}>Confirmed</option><option value="paid" ${x.status==='paid'?'selected':''}>Paid / fulfilled</option><option value="declined" ${x.status==='declined'?'selected':''}>Declined</option></select></td></tr>`).join(''):'<tr><td colspan="6">No specific-need sponsorship selections yet.</td></tr>'}
      </tbody></table></div>
      <div class="admin-callout" style="margin-top:12px"><strong>Drive sync:</strong> these records are the live website source. The matching Google Drive sponsorship tracker will be reconciled from confirmed/paid records so pending selections never close a need prematurely.</div>`;
